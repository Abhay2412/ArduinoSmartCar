...
delay(2000);
setMotorSpeed(100);                                               //Set motor speed 100 out of 255
moveForward();
...
void loop() {
  ...
  gyroZ = gyroZ + gyroAccelTemp.getRotationZ() / 131.0 *1.76 * timeStep;         // Get current Z axis orientation in degrees (conversion from degrees per second
  ...
  Serial.println(gyroZ);
      ...
      while(forward){
             ...
             if(ultrasonicDistanceInCM() < 13){                            //If robot is closer than 13cm than turn left slightly
              ...
              gyroZ = gyroZ + gyroAccelTemp.getRotationZ() / 131.0 *10.49* timeStep;         // Get current Z axis orientation in degrees (conversion from degrees per second
    ...
    while(reverse){
          ...
          moveForward();
           delay(1);
                ...
                if(gyroZ >(-360*currentLapRev+180)){       //Keep in mind the gyroscope starts at -1800+180 degrees for the reverse
                  currentLapRev++;               
...
float z = 180;
      ...
      if(gyroZ > (gyroBefore +180)){
        ...
        reverse = true;
        currentLap = 7;                     //Set to 7 so turn around function is ensured to be turned off.
      }
